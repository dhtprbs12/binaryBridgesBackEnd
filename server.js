...
import scriptEmail from './assets/scriptEmail.js'
dotenv.config()
...
app.post('/send-script-to-email', async (req, res) => {
	const { email, scriptName } = req.body
	console.log(email, scriptName)
	const mailMeta = {
		from: 'binarybridgeonline@gmail.com',
		to: email,
		subject: 'Binary Bridge - Thank your purchasing. Here is the script',
		html: scriptEmail(email.slice(0, email.indexOf('@')), scriptName),
		attachments: [
			{
				filename: `${scriptName}.pdf`, // <= Here: made sure file name match
				path: path.join(__dirname, `/assets/${scriptName}.pdf`), // <= Here
				contentType: 'application/pdf',
			},
			{
				filename: `${scriptName}.docx`, // <= Here: made sure file name match
				path: path.join(__dirname, `/assets/${scriptName}.docx`), // <= Here
				contentType:
